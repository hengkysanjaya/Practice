...
namespace SearchRouteReview3
{
    public partial class Form1 : Form
    {
        ...
        private int Search(int From, int To, DateTime Time, int Transit,List<FlightSchedule> visited)
        {
            if (Transit > 4) return 0;
            if (visited.Where(x => x.From == From).Count() > 0) return 0;
            ...
            var q = flights.Where(x => x.From == From & x.Time > Time);
            int bisa = 0;
            foreach (var a in q)
            {
